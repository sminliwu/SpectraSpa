...
// returns true when received full message or timeout
  if (_sendRequest) {
    sendRqst(CMD_NUL, _request.data1, _request.data2); 
    return(false); // obvi not ready while req is being prepared
  } else {
    uint8_t c = '\0';
    ...
    if (!_Out.available()) return(false);
...
// returns true if the response status is ready for processing
  ...
  switch (_reqStage) {
    case 0: // req stage 0: loading msg and sending on serial out
      if (cmd != CMD_NUL) {
        _sendRequest = true;
        ...
        _request.cmd = cmd;
        _request.data1 = data1;
        _request.data2 = data2;
        ...
        _timeSent = millis();
        _reqStage++;
      } break; // end req stage 0
      
    case 1: // req stage 1: after delaying 20ms, start waiting for a response
      ...
      if (_timeSent - millis() >= REQUEST_DELAY) {
        _timeSent = millis();
        _status.code = STS_OK;
        _waitResponse = true;
        _sendRequest = false;
        if (_synch) { _reqStage++; } else { _reqStage = 0; }
      } break; // end req stage 1

    case 2: // req stage 2, only in sync mode when waiting on a request to be resolved
      _reqStage = 0;
      break;
  }
